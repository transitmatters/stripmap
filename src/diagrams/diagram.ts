...
const getStationDisplacementMap = (paths: Path[], stationsByRangeName: Record<string, Station[]>) => {
...
export class Diagram {
    ...
    private getAdjacentStationId(path: Path, displacement: number, after: boolean): null | string {
        ...
        for (const [stationId, stationDisplacement] of Object.entries(stations)) {
            const isOnCorrectSide = after ? stationDisplacement >= displacement : displacement >= stationDisplacement;
            if (isOnCorrectSide) {
    ...
    project(point: Point): null | DiagramProjection {
        ...
        if (closestPath && closestProjection) {
            const fromStationId = this.getAdjacentStationId(closestPath, closestProjection.displacement, false);
            const toStationId = this.getAdjacentStationId(closestPath, closestProjection.displacement, true);
            return {
