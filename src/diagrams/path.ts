...
const getRangesIndex = (ranges: RangeNames[]): RangesByName => {
    ...
    for (const name of allNames) {
        ...
        const lastIndexWithName =
            lastIndexInRestWithoutName === -1 ? ranges.length - 1 : firstIndexWithName + lastIndexInRestWithoutName;
...
export class Path {
    ...
    cut(fromDisplacement: number, toDisplacement: number) {
        ...
        const { segment: toSegment, index: toIndex, displacement: displacementInToSegment } = this.seek(toDisplacement);
    ...
    offset(distance: number) {
        ...
        return new Path(this.segments.map((segment) => segment.offset(distance) as unknown as Bezier).flat());
    }
    ...
    project(point: Point): null | PathProjection {
        ...
        if (closestProjection && closestSegment) {
            ...
            const displacementBeforeSegment = indexOfSegment === 0 ? 0 : this.accumulatedLengths[indexOfSegment - 1];
